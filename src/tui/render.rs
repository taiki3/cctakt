...
pub fn ui(f: &mut Frame, app: &mut App) {
    ...
    match app.mode {
        ...
        AppMode::TaskComplete => {
            render_task_complete(f, app, f.area());
        }
...
pub fn render_build_confirmation(f: &mut Frame, app: &App, area: ratatui::layout::Rect) {
    let t = theme();
    ...
    let branch_name = app.pending_build_branch.as_deref().unwrap_or("unknown");
...
pub fn render_task_complete(f: &mut Frame, app: &App, area: ratatui::layout::Rect) {
    let t = theme();
    ...
    let state = app.task_complete_state.as_ref();
    let (branch, message, build_info) = match state {
        Some(s) => {
            let build_status = if s.build_run {
                if s.build_success == Some(true) {
                    "Build: Success"
                } else {
                    "Build: Failed"
                }
            ...
            (s.branch.as_str(), s.message.as_str(), build_status)
        }
        None => ("unknown", "Task completed", ""),
    };
    ...
    let text = vec![
        ...
        Line::from(Span::styled(build_info, t.style_text_secondary())),
        Line::from(""),
        Line::from(Span::styled(
            message,
            t.style_success(),
        ...
        Line::from(""),
        Line::from(vec![
            Span::styled("[Enter/Esc]", t.style_key()),
    ...
    let block = Block::default()
        .borders(Borders::ALL)
        .border_style(Style::default().fg(t.neon_green()))
        .style(t.style_dialog_bg())
        .title(Span::styled(
            " Task Complete ",
            Style::default()
